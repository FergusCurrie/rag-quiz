...
def create_concept(db: Session, concept: str, user_created: bool = False) -> models.ConceptStore:
    ...
    db_concept = models.ConceptStore(concept=concept, user_created=user_created)
    db.add(db_concept)
    db.commit()
...
def get_concept(db: Session, concept_id: UUID) -> models.ConceptStore | None:
    ...
    return db.query(models.ConceptStore).filter(models.ConceptStore.concept_id == concept_id).first()


def get_concept_by_name(db: Session, concept: str) -> models.ConceptStore | None:
    """Retrieve a concept by its name"""
    return db.query(models.ConceptStore).filter(models.ConceptStore.concept == concept).first()


def update_concept(
    db: Session, concept_id: UUID, concept: str | None = None, user_created: bool | None = None
